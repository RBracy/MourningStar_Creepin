        ...
        // if no backup creep is required
            ...
            if (room.memory.logisticsEnabled == true) {
                for (let source of sources) {
                    ...
                    if (!_.some(creepsInRoom, c => c.memory.role == 'logistics' && c.memory.sourceId == source.id)) {
                        // check whether or not the source has a container
                        /** @type {Array.StructureContainer} */
                        let containers = source.pos.findInRange(FIND_STRUCTURES, 1, {
                            filter: s => s.structureType == STRUCTURE_CONTAINER
...
StructureSpawn.prototype.createCustomCreep =
    function (energy, roleName) {
        ...
        return this.spawnCreep(body, roleName + '_' + Game.time, { memory: { role: roleName, working: false, home: this.room.name } });
    };
...
StructureSpawn.prototype.createClaimHolder =
    function (target) {
        return this.spawnCreep([CLAIM, CLAIM, MOVE, MOVE], 'claimHolder' + target + Game.time, { memory: { role: 'claimHolder', target: target, home: this.room.name } });
    };

// Claimer (For claiming controllers)
StructureSpawn.prototype.createClaimer =
    function (target) {
        return this.spawnCreep([CLAIM, MOVE], 'claimer_' + Game.time, { memory: { role: 'claimer', target: target, home: this.room.name } });
    };
...
StructureSpawn.prototype.createInvader =
    function (target) {
        return this.spawnCreep([MOVE, MOVE, MOVE, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, ATTACK, MOVE, MOVE, MOVE], 'invader' + target + '_' + Game.time, { memory: { role: 'invader', target: target, home: this.room.name } });
    };

// Remote Constructor (I send this in to rooms I've reserved 
//  and am harvesting from to build roads/containers)
StructureSpawn.prototype.createRemoteConstructor =
    function (target) {
        return this.spawnCreep([MOVE, CARRY, WORK, WORK, CARRY, MOVE], 'remoteConstructor' + target + '_' + Game.time, { memory: { role: 'remoteConstructor', working: false, target: target, home: this.room.name } });
    };
...
StructureSpawn.prototype.createLorry =
    function (energy) {
        ...
        return this.spawnCreep(body, 'lorry_' + Game.time, { memory: { role: 'lorry', working: false, home: this.room.name } });
    };
...
StructureSpawn.prototype.createLogistics =
    function (sourceId) {
        return this.spawnCreep([MOVE, CARRY, CARRY, CARRY, CARRY, CARRY, MOVE, CARRY, CARRY, CARRY, CARRY, CARRY, MOVE], 'logistics_' + Game.time,
            { memory: { role: 'logistics', sourceId: sourceId, working: false, home: this.room.name } });
};
...
StructureSpawn.prototype.createSchlepper =
    function () {
